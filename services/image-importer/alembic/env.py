...
def run_migrations_offline() -> None:
    ...
    """
    # Override the URL from alembic.ini with your settings
    configuration = config.get_section(config.config_ini_section, {})
    configuration["sqlalchemy.url"] = settings.database_url.replace(
        "+asyncpg", ""  # ← alembic needs sync driver, not asyncpg
    )
    ...
    with connectable.connect() as connection:
        context.configure(
            ...
            include_schemas=True,  # ← needed because you use pony_image schema
        )
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        version_table="alembic_version",  # ← explicit name (optional but clear)
        version_table_schema="pony_image",
    )
    with context.begin_transaction():
        context.run_migrations()
